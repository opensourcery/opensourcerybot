...
exports.run = function (client, message, requires) {
    ...
    if (result[1] && result[2]) {
      ...
      if (groups.hasOwnProperty(group)) {
        if (groups[group][user]) {
          client.say(message.to, 'Group ' + group + ' already has user ' + user + '!')
        }
        else {
          groups[group].push(user)
          client.say(message.to, 'User ' + user + ' added to group ' + group)
        ...
        groups[group] = []
        groups[group].push(user)
        client.say(message.to, 'Group ' + group + ' created, and user ' + user + ' added to it')
        return {status:"update",file:groupsfile,data:groups}
      }
    }
    else {
  ...
  if (result) {
    ...
    if (groups.hasOwnProperty(group)) {
      ...
      client.say(message.to, 'I couldn\'t find that group, ' + message.from)
    }
    return {status:"success"}
  }
