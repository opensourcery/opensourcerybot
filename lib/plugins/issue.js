...
exports.run = function (client, message, requires) {
  ...
  github.authenticate({
    type: "basic",
    username: client.config.github.user,
    password: client.config.github.password
  })
  ...
  if (result && result[1]) {
    ...
    github.issues.create(issue, function(error, result) {
      if (error) {
        ...
        client.say(message.to, 'Sorry, ' + message.from + '. github rejected that issue!')
      }
      else {
        console.log('Issue created: ' + result)
        client.say(message.to, 'Thanks, ' + message.from + '. I know I have problems sometimes!')
      }
    })
