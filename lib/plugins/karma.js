...
var user
  ...
  if (result) {
    ...
    if (!storedkarma.hasOwnProperty(user)) {
      ...
      storedkarma[user].given = 0
      storedkarma[user].taken = 0
    }
    if (!storedkarma.hasOwnProperty(judge)) {
      storedkarma[judge] = {}
      storedkarma[judge].up = 0
      storedkarma[judge].down = 0
      storedkarma[judge].given = 0
      storedkarma[judge].taken = 0
    }
    if (!storedkarma.hasOwnProperty(handle)) {
      storedkarma[handle] = {}
      storedkarma[handle].up = 0
      storedkarma[handle].down = 0
      storedkarma[handle].given = 0
      storedkarma[handle].taken = 0
    }
    if (axis == '++' && user != message.from) {
      ...
      if (requires.functions.randInt(0,12) === 5 && lastaward != judge) {
        ...
        client.say(message.to, judge + '++ for keeping the karma rolling.')
