...
exports.run = {
  onmessage: function (client, message, requires) {
    ...
    if (result) {
      ...
      // Grab any messages after the actual functionality.
      reason_test = /^(?:\S+?)\s*(?:\+\+|--)(?:\s+)?(?:[0-9]+)?d?(?:[0-9]+)?(?:\s+)?(.*)$/.exec(message.content);

      if (reason_test[1]) {
        reason = ' "' + reason_test[1] + '"';
      }
      ...
      if (result[4]) {
        ...
        if (functions.randRoll) {
          ...
          // If the total amount is greater than 20 or is a d1, reroll with a d20.
          if(amount > 20 || dice == 1) {
            amount = functions.randRoll(20);
          }
          if (axis === '++') {
            client.respond(message, amount + ' karma randomly given to ' + recipient + reason);
          }
          else {
            client.respond(message, amount + ' karma randomly taken from ' + recipient + reason);
          }
        }
        else {
      ...
      users.forEach(function(user) {
        ...
        if (axis === '++' && user !== message.from) {
          storedkarma = exports.functions.karmaAdd(client, requires, message, user, amount, reason);
        }
        else if (axis === '++') {
          client.respond(message, 'Hey, ' + message.from + '! You can\'t give karma to yourself!');
        }
        else if (axis === '--') {
          storedkarma = exports.functions.karmaRemove(client, requires, message, user, amount, reason);
        }
      });
      ...
      if (axis === '++') {
        if (requires.functions.randInt(0,12) === 5 && lastaward !== message.from) {
          ...
          client.respond(message, message.from + '++ for keeping the karma rolling.');
...
exports.functions = {
   ...
   *  reason (string) Optional parameter that shows the stated reason for karma.
   *
   * @return (object) Adjusted storedkarma object
   */
  karmaAdd: function(client, requires, message, to, amount, reason) {
    var storedkarma = requires.karma.storedkarma;
    functions = requires.functions;
    ...
    console.log(message.from + ' awarded ' + amount + ' karma to ' + to + reason);
    // Tell the user they got karma (only if they're not online at this time).
    if (functions.tell && functions.checkOnline) {
      if (functions.checkOnline(client, [to], [message.to])[0] != to) {
        console.log('Storing karma tell for ' + to);
        storedTells = requires.karma.storedtells;
        newTell = {
          to: to,
          from: client.config.handle,
          message: 'You received ' + amount  + ' karma from ' + message.from + reason + '.',
          timestamp: Date.now()
        };
   ...
   * @return (object) Adjusted storedkarma object
   */
  karmaRemove: function(client, requires, message, to, amount, reason) {
    var storedkarma = requires.karma.storedkarma;
    var user = message.from;
    functions = requires.functions;

    if (to === user) {
      client.respond(message, 'If you say so, ' + user + '...');
    }
    else {
      storedkarma[user].taken += amount;
    }
    storedkarma[to].down += amount;
    console.log(user + ' took ' + amount + ' karma from ' + to + reason);
    // Tell the user they got karma (only if they're not online at this time).
    if (functions.tell && functions.checkOnline) {
      if (functions.checkOnline(client, [to], [message.to])[0] != to) {
        console.log('Storing karma tell for ' + to);
        ...
        newTell = {
          to: to,
          from: client.config.handle,
          message: message.from + ' took ' + amount  + ' karma from you' + reason + '.',
          timestamp: Date.now()
        };
        functions.tell(newTell, storedTells);
      }
