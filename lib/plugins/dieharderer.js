...
exports.run = {
  onmessage: function (client, message, requires) {
    ...
    // Only proceed every one-in-sixteen times
    if (requires.functions.randInt(0,16) !== 2) {
      return {status:'fail'};
    }
    ...
    if (result) {
      client.respond(message, harderify(result[1], 'Hard'));
    ...
    if (result) {
      client.respond(message, harderify(result[1], result[2]));
      return {status:'success'};
    }
    ...
    if (result) {
      client.respond(message, pick_style(result));
      return {status:'success'};
    }
