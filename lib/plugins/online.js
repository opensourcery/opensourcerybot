...
exports.functions = {
   ...
   *  names (array) - The optional array of users to check for online status.
   *
   * @return an array of users online, either of all users or pruned from the
   * names parameter.
   */
  checkOnline: function(client, names) {
    var online_users = [];
    var channels = client.config.params.channels;
    if (channels.length > 0) {
      for (var key in channels) {
        // Loop through the channels and their individual user lists.
        var channel_users = client.chanData(channels[key]).users;
