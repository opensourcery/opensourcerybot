...
exports.run = {
  onmessage: function (client, message, requires) {
    ...
    if (result && result[1] && result[2]) {
      ...
      if (exports.functions.aliasFindId(requires, newalias) !== false) {
        // Alias is taken.
        client.speak(message, 'Sorry, ' + message.from + ', ' + newalias + ' is already aliased!');
      }
      else if (exports.functions.aliasFindId(requires, oldalias) !== false) {
        // Name is already an alias, so find the userid of its alias group.
        userid = exports.functions.aliasFindId(requires, oldalias);
        if (aliasbyid[userid]) {
...
exports.functions = {
   ...
   */
  aliasFindId: function(requires, alias) {
    var aliasbyid = requires.alias.storedalias;
  ...
  /**
   * Find the first registered alias for a userID.
   *
   * @params
   *  requires (object) The standard osbot requires object.
   *  to (string) The alias to find the user ID of.
   ...
   * @return (int) the ID of the user.
   */
  aliasFindAlias: function(requires, userid) {
    var aliasbyid = requires.alias.storedalias;
    var idbyalias = exports.functions.aliasSortByAlias(aliasbyid);
    var alias = false;

    if (aliasbyid[userid] && aliasbyid[userid].length > 0) {
      alias = aliasbyid[userid][0];
      console.log('First found alias is ' + alias);
    }
