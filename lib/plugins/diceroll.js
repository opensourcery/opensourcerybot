  ...
  onmessage: function (client, message) {
    ...
    if (result) {
      if (result[2]) {
        ...
        // If crit fail or win.
        if (roll === multiplier) {
          client.respond(message,  'Crit fail.');
        }
        else if (roll == dicesize * multiplier) {
          client.respond(message,  'Crit hit!1! Pass the Cheetos.');
        }

        // Output message.
        if (typeof story !== 'undefined') {
          client.respond(message,  message.from + ' rolls a ' + roll + ' ' + story + '!');
        }
        else {
          client.respond(message,  message.from + ' rolls a ' + roll + '!');
        }
        return {status:"success"};
      }
      else {
        client.respond(message, '!diceroll doesn\'t work like that, ' + message.from + '!');
        return {status:"fail"};
      }
    } //end result
    return {status:"fail"};
