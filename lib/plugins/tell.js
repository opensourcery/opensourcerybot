  ...
  onmessage: function (client, message, requires) {
    ...
    if (result) {
      if (result[1] && result[2]) {
        ...
        if (tellResult) {
          client.say(message.from, user + ' will be notified when they next log in or speak, ' + message.from + '.');
          return {status:'update',file:tellsfile,data:storedtells};
        }
        else {
          client.respond(message, message.from + ', I\'m sorry, but there was an error, please try again later.');
          return {status:"fail"};
        }
      }
      else {
        client.respond(message, '!tell doesn\'t work like that, ' + message.from + '!');
      }
    }
    ...
    if (result && result[1]) {
      ...
      if (storedtells.hasOwnProperty(user)) {
        client.respond(message, 'User ' + user + ' has ' + storedtells[user].length + ' tells stored.');
      }
      else {
        client.respond(message, 'I have no tells stored for user ' + user + '.');
      }
    ...
    if (storedtells.hasOwnProperty(message.from) && storedtells[message.from].length > 0) {
      ...
      client.say(message.from, 'Hold on, I have stored messages for you, ' + message.from + '!');
      for (var i = 0;i < storedtells[message.from].length;i++) {
        tell = storedtells[message.from][i];
        timestamp = new Date(tell.timestamp);
        client.say(message.from, 'From ' + tell.from + ' on ' + timestamp.toString() + ': ' + tell.message);
      };
