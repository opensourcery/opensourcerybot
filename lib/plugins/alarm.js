  ...
  onmessage: function (client, message) {
    ...
    if (result) {
      if (result[1] && result[2]) {
        ...
        client.respond(message, 'Setting alarm ' + (event ? '"' + event + '" ' : '') + 'for ' + hour + ':' + minute + (month ? ' on ' + (month+1) + '/' + day : ' today') + '.');
      }
      else {
        client.respond(message, 'I didn\'t understand that alarm command, ' + message.from);
      }
...
function setAlarm(name, hour, minute, month, day, client, message) {
  ...
  setTimeout(function() {
    client.respond(message, 'ALARM, ' + message.from + '! It is now time for ' + (name !== '' ? name : 'whatever') + '!');
  }, interval);
}